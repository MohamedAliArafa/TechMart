...
//        context = TechMartApp.getAppContext();
    ...
    void loginLinkedin(String Id, Context context) {
        ...
        String firebaseToken = AppUtils.getFirebaseToken();
        try {
            JSONObject registerBody = MainApiHelper.createLoginLinkedBody(Id);
            MainApi.loginLinkedUser(registerBody, new NetworkResponseListener<UserData>() {
                @Override
                public void networkOperationSuccess(NetworkResponse<UserData> networkResponse) {
                    if (isDetachView()) return;
                    dialogsLoading.dismiss();
