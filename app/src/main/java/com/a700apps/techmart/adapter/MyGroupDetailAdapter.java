...
public class MyGroupDetailAdapter extends RecyclerView.Adapter<MyGroupDetailAdapter.ViewHolder> {

    private List<UserGroup> mUserGroupList;
    private final Context context;

    public MyGroupDetailAdapter(Context context, List<UserGroup> UserGroupList) {
        this.context = context;
        this.mUserGroupList = UserGroupList;
    }

    @Override
    public MyGroupDetailAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        Context context = parent.getContext();
        LayoutInflater inflater = LayoutInflater.from(context);
        View noteView = inflater.inflate(R.layout.my_groub_list_item, parent, false);

        return new MyGroupDetailAdapter.ViewHolder(noteView);
    }

    @Override
    public void onBindViewHolder(MyGroupDetailAdapter.ViewHolder viewHolder, int position) {
